...
<body>
	<div id="container">
		...
		<div id="content">
			...
			<option value="writerSearch">글쓴이</option>  <!-- value시 1991년이 아니라 value의 값이 넘어감  -->
			<option value="titleSearch">제목</option>
				...
				<table class="tbl-ex">
					...
					<tr>
						...
						<td style="padding-left:${30*vo.depth }px"><img src="<c:url value='/' />assets/images/reply.png"/><a href="<c:url value='/' />board/view?no=${vo.no}&userNo=${vo.userNo}">${vo.title}</a></td>
						</c:when>
						<c:otherwise>
						<td style="padding-left:${30*vo.depth }px"><a href="<c:url value='/' />board/view?no=${vo.no}&userNo=${vo.userNo}">${vo.title}</a></td>
						</c:otherwise>
						</c:choose>
						<td>${vo.userName}</td>
						<td>${vo.hit}</td>
